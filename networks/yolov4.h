...
namespace yolov4 {
    ...
    static const int INPUT_H = Yolo416::INPUT_H;
    static const int INPUT_W = Yolo416::INPUT_W;
    static const int DETECTION_SIZE = sizeof(Yolo416::Detection) / sizeof(float);
    static const int OUTPUT_SIZE = Yolo416::MAX_OUTPUT_BBOX_COUNT * DETECTION_SIZE + 1;  // we assume the yololayer outputs no more than MAX_OUTPUT_BBOX_COUNT boxes that conf >= 0.1
    ...
    ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, const std::string &weightsPath) {
        ...
        IConvolutionLayer* conv138 = network->addConvolutionNd(*l137->getOutput(0), 3 * (Yolo416::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.138.Conv2d.weight"], weightMap["module_list.138.Conv2d.bias"]);
        assert(conv138);
        // 139 is yolo layer
        ...
        IConvolutionLayer* conv149 = network->addConvolutionNd(*l148->getOutput(0), 3 * (Yolo416::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.149.Conv2d.weight"], weightMap["module_list.149.Conv2d.bias"]);
        assert(conv149);
        // 150 is yolo layer
        ...
        IConvolutionLayer* conv160 = network->addConvolutionNd(*l159->getOutput(0), 3 * (Yolo416::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.160.Conv2d.weight"], weightMap["module_list.160.Conv2d.bias"]);
        assert(conv160);
        // 161 is yolo layer

        auto creator = getPluginRegistry()->getPluginCreator("Yolo416Layer_TRT", "1");
